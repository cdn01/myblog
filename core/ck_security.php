...
class ck_security
{
	...
	public function __construct()
	{
		...
		{
			if (false !== ($val = ck_blog::get_instance()->config()->item($key)))
			{
				$this->{'_'.$key} = $val;
			}
		}
		...
		if (ck_blog::get_instance()->config()->item('cookie_prefix'))
		{
			$this->_csrf_cookie_name = ck_blog::get_instance()->config()->item('cookie_prefix') . $this->_csrf_cookie_name;
		}
	...
	public function csrf_set_cookie()
	{
		...
		$secure_cookie = (ck_blog::get_instance()->config()->item('cookie_secure') === true) ? 1 : 0;
		...
		setcookie($this->_csrf_cookie_name, $this->_csrf_hash, $expire, ck_blog::get_instance()->config()->item('cookie_path'), ck_blog::get_instance()->config()->item('cookie_domain'), $secure_cookie);
	...
	{
		return $this->entity_decode($match[0], strtoupper(ck_blog::get_instance()->config()->item('charset')));
	}
...
// END ck_security Class

/* End of file ck_security.php */
/* Location: ./core/ck_security.php */