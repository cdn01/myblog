...
$search_sql = "select * from assi_account where useful = 0 order by zan asc limit 1";
$assi_account = query($search_sql);
$update_sql = "update assi_account set zan=zan +1 where id='".$assi_account[0]['id']."'";
...
if(strpos($result, '{"ok":1')){ 
	...
	$update_sql = "update assi_account set zan=zan -1 where id='".$assi_account[0]['id']."'";
	mysql_query($update_sql);
	$update_sql = "update auto_weibo_create set zannum=zannum -1 where id='".$assi_account[0]['id']."'";
	mysql_query($update_sql);
	tmail($param['uname']."_assi_".time());
}
print_r($result);

?> 
<script type='text/javascript'>
	 setTimeout('location.href="assi.php"',1000*60);
</script>